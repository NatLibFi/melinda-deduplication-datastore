...
ACI_NAME="deduplication-datastore-ubuntu"
ACI_VERSION="1.0.0"
...
$ACBUILD_CMD mount add data /opt/melinda-deduplication-datastore/data
$ACBUILD_CMD mount add --read-only conf /opt/melinda-deduplication-datastore/conf

$ACBUILD_CMD copy aci-build/app /opt/melinda-deduplication-datastore/app
...
$ACBUILD_CMD run --engine $ACBUILD_ENGINE --working-dir /opt/melinda-deduplication-datastore/app -- /bin/bash -c 'npm install --production'
$ACBUILD_CMD run --engine $ACBUILD_ENGINE --working-dir /opt/melinda-deduplication-datastore/app/melinda-deduplication-common -- /bin/bash -c 'npm install --production'
...
$ACBUILD_CMD port add http tcp 8080
